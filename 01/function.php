...
function getProductOne($p_id){
  ...
  try {
    ...
    $dbh = dbConnect();
    // SQL文作成
    $sql = 'SELECT p.id, p.name, p.comment, p.price, p.pic1, p.pic2, p.pic3, p.user_id, p.create_date, p.update_date, c.name AS category FROM product AS p LEFT JOIN category AS c ON p.category_id = c.id WHERE p.id = :p_id AND p.delete_flg = 0';
...
function uploadImg($file, $key){
  ...
  if(isset($file['error']) && is_int($file['error'])){
    try {
      ...
      // 「UPLOAD_ERR_OK」などの定数はphpでファイルアップロード時に自動的に定期される。定数には値として０や１などの数値が入っている。
      switch ($file['error']) {
        case UPLOAD_ERR_OK: // OK
            break;
        case UPLOAD_ERR_NO_FILE: // ファイル未選択の場合
            throw new RuntimeException('ファイルが選択されていません');
        case UPLOAD_ERR_INI_SIZE: // php.int定義の最大サイズが超過した場合
