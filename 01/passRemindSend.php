    ...
    if(empty($err_msg)){
      ...
      try {
        ...
        if($stmt && array_shift($result)){
          ...
          //メールを送信
          $from = 'kanekazu0206@gmail.com';
          $to = $email;
          $subject = '【パスワード再発行認証】 | WEBUKATUMARKET';
          // EOTはEndOfTextの略。ABCでもなんでもいい。先頭の<<<のあとの文字列と合わせること。最後のEOTの前後に空白など何も入れてはいけない。
...
パスワード再発行認証キー入力ページ :
http://localhost:8888/git-webservice/01/passRemindRecieve.php
認証キー : {$auth_key}
※認証キーの有効期限は30分となります

認証キーを再発行された場合は下記ページより再度再発行をお願い致します。
http://localhost:8888/git-webservice/01/passRemindsend.php
